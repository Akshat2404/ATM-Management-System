package Register;        									//Register Package                                
import java.util.*;
...
class Details
{
    ...
    String PIN;
    double balance;
    Details()                                             //Intializing details with Constructor with 0 arguments 
    {
        username = "";
        PIN = "";
        balance = 0.0;
    }
    Details(String username,String PIN,double balance)   //Constructor with 3 arguments will store the values of data entered by user
    {
        this();
	...
	void changeUsername(String username)                //For updating username
	{
		this.username = username;
	}	
	void changePIN(String PIN)                         	//For updating pin
	{
		this.PIN = PIN;
	}
}
...
{
    Details[] obj = new Details[50];					//An arrsy of 50 objects
    static int c = 0;									//Count of the number of accounts
    void setdetails()                                   //method to get username and pin from user to register
    {
        ...
        do                                           	//this do while loop will take the proper username from the user
        {
            ...
            name = sc.nextLine().trim();				//Scanning and trimmimg all the extra spaces
            if(isValidName(name))                    	
				//checking if username entered by user is valid or not
                flag = false;
			else										//If username already exists new account cannnot be made with the same name 
        ...
        do                                          //this do whie loop will take the proper pin number from the user
        {
            ...
            if(isValidPIN(pin))                     //checking if pin entered by user is valid or not
                flag = false;
			else									//PIN should be only 4 digit number
